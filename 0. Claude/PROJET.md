...
| mentions-legales.html, protection-des-donnees.html | ✅ faites | pages légales (legal.css). Les Conditions générales ont été supprimées le 2026-07-19 (inutiles sans vente en ligne) — leur clause « caractéristiques et photos indicatives » est fusionnée dans les Mentions légales (section « Instruments et informations »). Notes de vérification retirées. TVA : non assujetti (< 100 000 CHF/an, confirmé par Simeon) — indiqué dans la section Statut ; section « Crédits » ajoutée le 2026-08-13 (conception + développement, lien vers simeon-b.ch), qui remplace le « pas de crédit de conception » décidé le 2026-07-19.

## Reste à faire (phases)

Toutes les pages sont faites, toutes les images sont livrées, le domaine est en ligne. Reste :

0. ✅ **FAIT le 2026-08-13 — « Prix sur demande » sur les 3 pages instrument.** Un `<p class="instrument-price">` en **dernier enfant de `.instrument-info`** — cette seule position sert les deux mises en page : en desktop il tombe en bas de la colonne de texte, en mobile sous le bloc de texte et **avant la grande photo** (qui est en `order: 2`). **Aucun filet** : une simple ligne de texte courant, taille et graisse **héritées du body** (20px/32 en desktop, 16px/30 en mobile via la règle des paragraphes), seule la **couleur orange** la distingue. **36px au-dessus** du mot — obtenus par `margin-top: calc(36px - 60px)` en desktop, qui **compense le `gap: 60px`** de `.instrument-info` (sans ça on aurait 96px) ; en mobile le gap vaut déjà 36px, donc `margin-top: 0`. Vérifié par mesure DOM sur les 3 pages, à 1440 et 375 : 36px exactement au-dessus, typographie **identique au paragraphe voisin** (seule la couleur diffère), `border-top: 0`, 36px avant la photo en mobile, aucun débordement horizontal. ⚠️ **Premier jet remplacé le même jour** : il avait un filet orange, 32px ExtraBold condensé et 200px de marge (d'après une capture de Simeon) — Simeon a demandé la version sobre ci-dessus. ⚠️ Le panneau desktop grandit : le défilement différencié s'y adapte tout seul (`calibrate()` mesure `panel.getBoundingClientRect().height`) — vérifié avec une version **encore plus haute** que la version retenue, le bas du panneau finissait **pile à `END_GAP` = 60px** au-dessus du footer sur les 3 pages, et la bascule de bois recalait sans saut.
1. ✅ **FAIT le 2026-08-13 — Titres « selon [facteur] »** (facteurs donnés par Simeon) : « Hautbois en do **selon Jeremias Schlegel** en 415 Hz », « Alto en fa **selon P.J. Bressan** en 415 Hz », « Soprano en do **selon B. Reich** en 415 Hz ». Appliqué aux **H1 des 3 pages instrument** et, pour rester cohérent, aux **6 titres d'instruments.html** (3 entêtes desktop `.instr-head` + 3 cartes mobiles) qui portaient la même formule en « de ». Vérifié à 1440 : les 3 entêtes tiennent toujours sur **une seule ligne** (46px de haut, la plus large à 1033px), et en mobile les `<br>` du H1 restent masqués (titre qui s'écoule). ⚠️ **Non touchés** : `<title>`, `og:title`, meta descriptions et JSON-LD, qui disent « d'après [facteur] » — même sens, et les titles ont été calibrés sous 60 signes le 2026-08-11. À aligner seulement si Simeon le demande.
2. ✅ **FAIT le 2026-08-13 — Crédit/lien vers le site de Simeon**, dans les **Mentions légales uniquement**, section « Crédits » après « Propriété intellectuelle » : « Conception graphique et développement : Simeon Brandner — [www.simeon-b.ch](https://www.simeon-b.ch/) » (`target="_blank" rel="noopener"`).
   - ⚠️ Annule la décision du 2026-07-19 « pas de crédit de conception ».
   - **Pas de crédit dans le footer** : une variante avait été posée (copyright à gauche `1 / 3`, `.footer-credit` à droite `3 / 5`, `order: 6` centré en mobile) puis **retirée à la demande de Simeon** — le footer est revenu à l'identique (copyright seul en `grid-column: 3 / 5`). Ne pas la reproposer.
   - ✍️ **Le prénom s'écrit « Simeon », sans accent** — jamais « Siméon ».
3. ⏳ **OUVERT — Atelier (mobile) : les têtes de flûtes glissent encore un peu trop vite** (retour de Simeon le 2026-07-25, après la mise en ligne). Le réglage du 2026-07-25 (`SLIDE_EASE = 3`, `SLIDE_END_MARGIN = 0.18`) a bien adouci le **départ**, mais l'impression générale reste trop rapide.
...
4. **Responsive** d'après les frames mobiles de Simeon — aussi un enjeu SEO : Google indexe en mobile-first.
...
5. **Responsive** d'après les frames mobiles de Simeon — aussi un enjeu SEO : Google indexe en mobile-first.
6. **Finitions SEO/qualité** : voir checklist ci-dessous (canonical/sitemap/JSON-LD débloqués maintenant que le domaine est live), Open Graph, Lighthouse, validation HTML
   ...
   - ✅ **Mobile, menu : « Instruments » va droit à la page — corrigé le 2026-08-13** (retour de Simeon : « il faut cliquer deux fois »). ⚠️ La vérification du 2026-07-25 (« `.site-nav .sub-menu { display: none }` dans le bloc `@media (max-width: 900px)`, rien à corriger ») **était fausse** : elle avait regardé la règle sans comparer les spécificités. `.site-nav .has-sub:hover .sub-menu` / `:focus-within` valent **0,4,0** contre **0,2,0** pour `.site-nav .sub-menu` → la règle desktop l'emportait **aussi dans la media query**. Sur écran tactile, le 1ᵉʳ appui sur « Instruments » ne faisait donc que poser l'état `:hover` (le sous-menu s'affichait à `top: 100%` de `.site-nav` en `fixed`, soit **juste sous la fenêtre** — invisible) et le navigateur n'activait pas le lien ; il fallait un 2ᵉ appui. Correctif dans le bloc mobile de `layout.css` : les trois sélecteurs (`.sub-menu`, `.has-sub:hover .sub-menu`, `.has-sub:focus-within .sub-menu`) passent ensemble en `display: none`. Vérifié par mesure DOM à 375px : `display` reste **`none`** avec le lien focalisé (donc à l'état survol), hauteur 0 ; desktop 1280px intact (panneau 245×187 sous le menu, les 3 entrées présentes). ⚠️ Piège de préview rencontré : `python3 -m http.server` n'envoie pas de `Cache-Control` → Chrome garde l'ancien CSS en cache heuristique ; recharger ne suffit pas, il faut un `fetch(..., {cache:'reload'})` ou un paramètre d'URL pour voir la correction.
...
11. **Favicon et vignette Google ✅ (2026-08-10)** — après une première apparition dans les résultats Google (globe générique + vignette = `gravure_flutes.webp`, pas la photo hero) :
    - **Favicon** : les deux défauts étaient (a) `/favicon.ico` en **404** — c'est le chemin que le crawler de favicons de Google demande — et (b) des tailles hors spec (32 et 512 déclarés ; Google veut un carré **multiple de 48 px**). Générés depuis `favicon-512.png` : `favicon-48/96/192.png` + un `favicon.ico` multi-tailles (16/32/48) placé dans `2. HTML/` et copié à la racine du Staging par une nouvelle ligne de `build.sh`. `favicon-32/512.png` restent sur le disque mais ne sont plus déclarés. ⏳ Google met des **semaines** à rafraîchir un favicon en cache.
    - **Vignette** : `max-image-preview:large` ajouté sur les 8 pages indexables (absent jusqu'ici → Google se limitait à une vignette réduite). Les 2 pages légales en `noindex` sont volontairement exclues.
    - ⚠️ **Piège rencontré** : ajouter `width`/`height` sur `.hero-photo` a cassé le hero (rendu 1280×**1621** au lieu de 1280×776) — l'attribut `height` s'applique comme hauteur CSS, et rien ne le contredisait. Corrigé par **`height: auto`** dans `home.css`. Réflexe à garder pour toute image dont la taille est pilotée en CSS.
    - Le choix de la vignette reste à Google : ces réglages améliorent les chances, ils n'imposent rien. Pour vraiment maîtriser l'image, il faudrait une **image dédiée en portrait ou carrée** montrant les instruments.
12. **Titles raccourcis sous 60 signes ✅ (2026-08-11)** — le résultat Google de la Home affichait un titre coupé : « Christoph Brandner, facteur d'instruments à vent baroques … », **« à Genève » sautait**, alors que c'est le mot-clé local le plus utile. Deux constats : Google coupe vers **60 signes** (6 pages sur 10 dépassaient, jusqu'à 94), et il **supprime lui-même le préfixe de marque** quand il fait doublon avec le nom de domaine affiché juste au-dessus. Nouveaux titles (60/59/53… signes), `og:title` aligné sur chacun :
...
- Nouvelle section « **Crédits** » dans `mentions-legales.html`, après « Propriété intellectuelle » : « Conception graphique et développement : Simeon Brandner — [www.simeon-b.ch](https://www.simeon-b.ch/) » (`target="_blank" rel="noopener"`). ⚠️ Annule la décision du 2026-07-19 « pas de crédit de conception ».
- Une variante **dans le footer de toutes les pages** avait été posée puis **retirée à la demande de Simeon** : le footer est revenu à l'identique (copyright seul). Détail et raison en §2 de « Reste à faire » — ne pas la reproposer.
- ✍️ Le prénom s'écrit « **Simeon** », sans accent.

### 2026-08-13 — Pages instrument : « Prix sur demande » + titres « selon [facteur] »

- **« Prix sur demande »** ajouté sur soprano/alto/hautbois. Un seul élément, `<p class="instrument-price">`, posé en **dernier enfant de `.instrument-info`** : comme `.instrument-media` est en `order: 2` sous 900px, cette position unique donne « bas de la colonne de texte » en desktop **et** « sous le texte, avant la photo » en mobile — aucune duplication de markup, aucun `display: none` croisé. **Deux passes dans la session** : d'abord d'après une capture de Simeon (filet orange, 32px ExtraBold condensé, 200px de respiration), puis **version sobre retenue** — pas de filet, taille et graisse du texte courant, 36px au-dessus, seule la couleur orange reste. Cotes en §0 de « Reste à faire ».
- ⚠️ **Le `gap` de `.instrument-info` se cumule à la marge** : il vaut 60px en desktop et 36px en mobile. Pour obtenir 36px des deux côtés, le desktop utilise `margin-top: calc(36px - 60px)` (négative, compensation explicite) et le mobile `margin-top: 0`. Toute nouvelle cote sur ces enfants doit tenir compte du gap, sinon on lit 96px là où on croit poser 36.
- **Titres** : « de [facteur] » → « **selon** [facteur] » sur les 3 H1, **et** sur les 6 titres d'instruments.html (entêtes desktop + cartes mobiles) qui reprenaient la même formule — laisser les deux formulations aurait été incohérent d'une page à l'autre. Les balises `<title>`/`og:title`/JSON-LD gardent « d'après », volontairement (voir §1).
- **Vérifié par mesure DOM** à 1440 et 375 sur les 3 pages : gaps 200/96px, filet aligné sur la colonne de texte (772px en desktop, 357px à 375), 18px sous le filet, 36px avant la photo en mobile, entêtes d'instruments.html toujours sur une ligne, aucun débordement horizontal, console propre.
- ⚠️ **Piège du défilement différencié** : le panneau desktop grandit de ~250px. Aucun ajustement n'a été nécessaire (`calibrate()` mesure la hauteur réelle du panneau), mais **attention en test** : recalibrer alors qu'on est déjà en **bas** de page fige la pente à 0 (`scrollMax − y = 0` dans `main.js`), et le panneau reste alors à `translate3d(0,0,0)` — j'ai cru deux fois à une régression avant d'identifier l'artefact. Tester en rechargeant puis en défilant, jamais en mesurant après un `scrollTo(bas)` qui déclenche un recalage.

### 2026-08-13 — Menu mobile : « Instruments » demandait deux appuis

- **Symptôme (Simeon)** : sur mobile, il fallait appuyer **deux fois** sur « Instruments » pour arriver sur la page ; il soupçonnait un reliquat du sous-menu desktop — c'était exactement ça.
- **Cause** : bataille de spécificités. Le bloc mobile posait bien `.site-nav .sub-menu { display: none }` (**0,2,0**), mais les règles desktop `.site-nav .has-sub:hover .sub-menu` et `…:focus-within .sub-menu` valent **0,4,0** et gagnent donc **même à l'intérieur de la media query** — une media query ne change pas la spécificité. Au 1ᵉʳ appui, le navigateur tactile posait `:hover`, le sous-menu passait en `display: block` (à `top: 100%` de `.site-nav`, qui est en `fixed` → juste **sous** la fenêtre, donc invisible) et, comme l'appui avait révélé du contenu, le lien n'était pas activé. Le 2ᵉ appui suivait le lien.
- **Correctif** (1 règle, `layout.css`, bloc `@media (max-width: 900px)`) : les trois sélecteurs sont mis en `display: none` ensemble, avec un commentaire expliquant le piège de spécificité pour qu'on ne le « simplifie » pas plus tard.
- **Vérifié** par mesure DOM : à 375px, sous-menu à `display: none` et hauteur 0 **même avec le lien focalisé** (état équivalent au survol) ; à 1280px le sous-menu desktop est intact (panneau 245×187 sous le menu, « Soprano / Alto / Hautbois 415 Hz »). Console propre. Buildé dans le Staging.
- ⚠️ **Piège de préview à retenir** : `python3 -m http.server` n'envoie aucun `Cache-Control`, Chrome applique donc un cache heuristique sur le CSS — après un `build.sh`, une simple navigation peut resservir l'**ancien** fichier. Vérifier avec un `fetch(url, {cache:'reload'})` puis `location.reload()`, ou un paramètre `?v=` (sinon on teste sa correction sur l'ancien CSS et on croit qu'elle ne marche pas).
